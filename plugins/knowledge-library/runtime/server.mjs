...
var UriTemplate = class _UriTemplate {
  ...
  match(uri2) {
    _UriTemplate.validateLength(uri2, MAX_TEMPLATE_LENGTH, "URI");
    let pattern = "^";
    const names = [];
    for (const part of this.parts) {
      if (typeof part === "string") {
...
var api = new KnowledgeAPI();
var server = new McpServer({ name: "knowledge-library", version: "0.1.0" });
var uri = "ui://knowledge-library/library";
for (const [name, definition] of Object.entries(TOOL_DEFINITIONS)) {
  ...
  server.registerTool(name, {
    ...
    try {
      ...
      return { isError: true, content: [{ type: "text", text: error.message || "\u8D44\u6599\u5E93\u64CD\u4F5C\u5931\u8D25\u3002" }] };
    }
  });
}
server.registerResource("library-panel", uri, { mimeType: "text/html;profile=mcp-app" }, async () => ({ contents: [{
  uri,
  mimeType: "text/html;profile=mcp-app",
  text: await readFile(new URL("./library.html", import.meta.url), "utf8"),
  _meta: { ui: { csp: { connectDomains: [], resourceDomains: [] } } }
}] }));
server.registerResource("knowledge-source", new ResourceTemplate("knowledge://documents/{document}/revisions/{revision}/chunks/{chunk}", { list: void 0 }), { mimeType: "text/plain" }, async (uri2, variables) => {
  ...
  return { contents: [{ uri: uri2.href, mimeType: "text/plain", text: JSON.stringify(data) }] };
});
