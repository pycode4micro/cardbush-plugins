...
空结果不等于零消耗。先确认经典或全域计划口径：`qianchuan_get_account_report` / `qianchuan_get_ad_report` 仅覆盖经典计划；全域使用 `qianchuan_get_uni_promotion_report`，先读取对应报表配置，不回退到经典报表。

根据已有证据区分日期无数据、没有活跃广告、查询口径不匹配。按需要核对请求日期与账户报表时区、相应类型的计划列表/详情、`marketing_goal`、主题、筛选和维度；不要改到历史有数日期来代替用户指定日期。当前无活跃广告也不能证明历史无消耗。证据不足时明确“本次查询未返回数据，尚不能确认消耗”，只有正确口径实际返回的零值才按零汇报。历史数据可用于说明，不可直接驱动当前 live 写入。
