...
Uses the official MediaKit fine endpoint with explicit v5 / Subtitle / Quality defaults. Includes unchanged local
uploads, offline previews, region/time filters, one-time submission, task queries and exact-byte downloads.
Reuses `MEDIAKIT_API_KEY`; no translation, dubbing or video regeneration. Input max 2K; output max 1080p.
See [subtitle erasure](docs/video-subtitle-erase.md) for the complete request contract and examples.

Migration: run `python -m pip install .`, enable `volcengine-plugins` and disable the old `seedream-mcp` connection.
The new module is `python -m volcengine_plugins` and the MCP server key is `volcengine`.
Existing image/video/enhancement tool names and environment variables are unchanged.
...
- Codex / Cardbush use the included `.codex-plugin/plugin.json` and `.mcp.json`. Add the market with `codex plugin marketplace add pycode4micro/cardbush-plugins --ref main`, then run `codex plugin add volcengine-plugins@cardbush-plugins`.
...
For local erasure inputs use `video_subtitle_erase_upload`; after completion use `video_subtitle_erase_download_task` to save exact result bytes to a new absolute path. See the [erasure guide](docs/video-subtitle-erase.md).
...
| Symptom | Action |
| --- | --- |
| `No module named volcengine_plugins` | Run `python -m pip install .` with the Python interpreter used by the client |
